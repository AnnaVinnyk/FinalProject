...
/*let Says = 'Lorem ipsum dolor sit, amet consectetur adipisicing elit. A, ad veniam officiisillum nostrum pariatur voluptatibus eveniet corrupti aperiam! Expedita nostrum earum quae voluptate temporibus distinctio totam unde neque soluta.'
//let n = Number(prompt("Введите желаемое количество блоков (любое целое положительное число): "));
let n = 20;
for (let i = 0; ; ++i) {
    //n = Number(prompt("Введите желаемое количество блоков (любое целое положительное число): "));
    if (Number.isInteger(n) && n > 0) {
        //alert('Карточки появляются...');
        //alert('...карточки появляются...');
        //alert('...карточки появились!');
        break;
    } else if (n == 0) { alert("Страница отобразится без карточек"); break; }
    else {
        alert('Значение введено не корректно!');
        continue;
    }
};
for (let i = 0; i < n; i++) {
    ...
    } else div.setAttribute('class', "sub1-blocks-item-temp");
    div.textContent = 'Кристофер Майкл Прэтт';
    ...
    img1.setAttribute('src', 'clients/client-008.jpg');
    img1.setAttribute('alt', 'Кристофер Майкл Прэтт');
    ...
    div4.textContent = 'Лемтранс';
    div3.appendChild(div4);
    let div5 = document.createElement('div');
    div5.setAttribute('class', "profession");
    div5.textContent = 'Региональный директор';
    ...
    img2.setAttribute('src', 'logos/logo-008.jpg');
    img2.setAttribute('alt', 'Лемтранс');
...
let Photos = ['clients/client-001.jpg', 'clients/client-002.jpg', 'clients/client-003.jpg', 'clients/client-004.jpg', 'clients/client-005.jpg', 'clients/client-006.jpg', 'clients/client-007.jpg', 'clients/client-008.jpg'],
    ...
    Caption01 = ['FERREXPO', 'МЕТІНВЕСТ', 'ДТЭК', 'ЧАО "Запорож' + "\u00AD" + 'трансформатор"', 'ПРАО "Донецксталь"', 'ЮГОК', 'АО "МОТОР СИЧ"', 'Лемтранс'],
